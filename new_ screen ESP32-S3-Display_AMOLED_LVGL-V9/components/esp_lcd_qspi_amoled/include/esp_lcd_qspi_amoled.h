 ...
 */
typedef struct {
    const qspi_amoled_lcd_init_cmd_t *init_cmds;    /*!< Pointer to initialization commands array.
...
} qspi_amoled_vendor_config_t;

/**
 * @brief Create LCD panel for model QSPI_AMOLED
 *
 ...
 *
 */
#define QSPI_AMOLED_PANEL_BUS_SPI_CONFIG(sclk, mosi, max_trans_sz)   \
    {                                                           \
        .sclk_io_num = sclk,                                    \
        .mosi_io_num = mosi,                                    \
        .miso_io_num = -1,                                      \
...
#define QSPI_AMOLED_PANEL_BUS_QSPI_CONFIG(sclk, d0, d1, d2, d3, max_trans_sz) \
    {                                                           \
        .sclk_io_num = sclk,                                    \
        .data0_io_num = d0,                                     \
        .data1_io_num = d1,                                     \
 ...
 *
 */
#define QSPI_AMOLED_PANEL_IO_SPI_CONFIG(cs, dc, cb, cb_ctx)          \
    {                                                           \
        .cs_gpio_num = cs,                                      \
        .dc_gpio_num = dc,                                      \
        .spi_mode = 0,                                          \
...
#define QSPI_AMOLED_PANEL_IO_QSPI_CONFIG(cs, cb, cb_ctx)             \
    {                                                           \
        .cs_gpio_num = cs,                                      \
        .dc_gpio_num = -1,                                      \
        .spi_mode = 0,                                          \
